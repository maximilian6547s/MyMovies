...
public class NetworkUtils {
    ...
    private static final String API_KEY = "a7b5271f61e2f90200769da19c4863d0";
    ...
    public static URL buildURLToVideos(int id, String lang) {
        URL result = null;
        Uri uri = Uri.parse(BASE_URL_VIDEOS).buildUpon()
                .appendQueryParameter(PARAMS_API_KEY, API_KEY)
                .appendQueryParameter(PARAMS_LANGUAGE, lang)
                .build();
        try {
            result = new URL(String.format(uri.toString(), id));
        } catch (MalformedURLException e) {
            e.printStackTrace();
        }
        return result;
